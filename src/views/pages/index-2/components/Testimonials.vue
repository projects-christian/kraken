<template>
    ...
    >
        <div class="container">
            <div
                class="bg-default-950/20 backdrop-blur-sm rounded-xl relative md:p-20 p-6"
            >
                ...
                >
                    <SwiperSlide v-for="(item, idx) in testimonials" :key="idx">
                        <div class="max-w-2xl mx-auto text-center">
                            ...
                            />
                            <p
                                class="text-lg font-medium text-default-200 mt-7"
                            >
                                "{{ item.review }}"
                            </p>
                            <img
                            ...
                            >
                                <Star
                                    v-for="(star, idx) in Array.from(
                                        new Array(Math.floor(item.rating ?? 0)),
                                    )"
                                    :key="idx"
                                    class="h-4 w-4 stroke-yellow-300 fill-yellow-300"
                                />

                                <StarHalf
                                    v-if="!Number.isInteger(item.rating)"
                                    class="h-4 w-4 stroke-yellow-300 fill-yellow-300"
                                />
